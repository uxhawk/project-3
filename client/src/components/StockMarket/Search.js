...
import CurrentSearch from '../../components/StockMarket/CurrentSearch';
...
const Search = () => {
  ...
  const getPrice = () => {
    ...
    const inputValueSplit = searchInputValue.split(' ');
    ...
    API.getCurrentStockPrice(inputSymbol)
        .then((res) => {
          ...
          res.data['Time Series (5min)'][mostRecentUpdateTime]['1. open'];
          dispatch({
  ...
  return (
    <div>
      <div className="row pb-4">
        <div className="col-lg-8 col-md-10 offset-lg-2
        offset-md-1 gradient p-3">
          <div className="input-group mb-3">
            <input list="availableStocks" type="text"
              className="form-control" id="stockSearch"
              placeholder="Search for stock prices" />
            <div className="input-group-append">
              <button className="btn btn-raised btn-white"
                type="button" id="button-addon3" onClick={() => {
                  getPrice();
        ...
        <div className="col-md-4 offset-md-4">
          {
                ...
                <CurrentSearch currentSearch={state.currentSearch}/>
          }
        </div>
      </div>
    </div>
