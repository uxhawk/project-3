...
const Card = (props) => {
    ...
    <div className="card gradient text-white" key={props.id}>
        ...
        <div className="card-body">
            ...
            <button id={props.id} className="btn btn-raised btn-white" symbol={props.state.symbol} onClick={ (event) => {handleRefreshClick(event)} }><ion-icon name="refresh-outline"></ion-icon><span className="ml-2">Refresh</span></button>
        </div>
    </div>
    );
};
