 ...
 * @param {event} event The target of the click event to get
 * goal ID.
 */
  ...
  return (
    ...
    align-items-center border-bottom mt-3">
      <div>
        <p className="p-0 m-0">
          <span className="bold">Goal Title:</span> {props.details.title}</p>
        <p className="p-0 m-0">
          <span className="bold">Goal Description:
          </span> {props.details.body}</p>
      </div>
      <button className="btn btn-raised btn-white"
        data-id={props.details.goalId} onClick={(event) => {
          handleDoneClick(event);
        }}><ion-icon name="checkmark-circle-outline"></ion-icon>
        <span className="ml-2">Done</span></button>
    </li>
  );
};
