...
const Login = () => {
  ...
  /**
   * function to direct the user to the signup page
   * @param {event} event gets the target to tell the router where to go
   */
  ...
  /**
   * function to launch the login authentication process
   * @param {event} event prevents default
   * @return {alert} if the user does not enter credentials
   */
  function handleLoginBtnClick(event) {
    ...
    // run the checkEmail function and clear the input fields
  ...
  /**
   * function to send user input to the backend for passport auth
   * @param {email} email from the input field
   * @param {password} password from the password field
   */
